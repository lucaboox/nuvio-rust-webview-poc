...
export function isDebridResolveStream(stream: StreamSource): boolean {
  return stream.clientResolve?.type?.trim().toLowerCase() === "debrid";
}

function isCachedDebridCandidate(stream: StreamSource): boolean {
  return isDebridResolveStream(stream) && stream.clientResolve?.isCached === true;
}

function passesDebridFilters(stream: StreamSource, settings: DebridRules): boolean {
...
function streamText(stream: StreamSource): string {
...
function streamResolution(stream: StreamSource): number {
...
function streamSize(stream: StreamSource): number {
  const raw = stream.clientResolve?.stream?.raw;
  return raw?.size ?? raw?.folderSize ?? stream.behaviorHints?.videoSize ?? 0;
}

function parsedHdr(stream: StreamSource): string[] {
...
function streamCodec(stream: StreamSource): "ANY" | "H264" | "HEVC" | "AV1" {
...
function qualityRank(stream: StreamSource): number {
