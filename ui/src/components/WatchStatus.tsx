...
export function WatchStatus({ state }: { state: WatchState | null }) {
  ...
  return <>{state.watched && <span className="watch-status watched"><b>✓</b></span>}{!state.watched && state.percent != null && <span className="watch-progress-track"><i style={{ width: `${state.percent}%` }} /></span>}</>;
}
